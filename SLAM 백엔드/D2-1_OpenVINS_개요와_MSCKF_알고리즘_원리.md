...
>
> 여기서 **Jacobian**은 "추정값을 조금 바꾸면 예측 측정값이 얼마나 바뀌는가"의 비율을 모아둔 기울기표이고, **영공간(null space)**은 "그 값을 바꿔도 결과에 아무 영향이 없는 방향들"의 모음이다. 특징점 위치 방향의 영공간에 투영한다는 것은 곧 **특징점 위치가 얼마든 상관없어지도록 만든다**는 뜻이다. (더 자세한 배경은 [[D0-1_D_시리즈를_위한_최소_수학|D0-1]] 4장 참고)
