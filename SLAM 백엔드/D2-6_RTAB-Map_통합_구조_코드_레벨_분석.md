...
- D2-5에서 다룬 ZUPT 오발동 가설이 만약 사실이라면, 이 `reset()` 무시 동작 때문에 **한 번 "조용한 정지" 상태에 빠지면 RTAB-Map이 아무리 재초기화를 요청해도 스스로 빠져나오지 못할 가능성**이 있다 — 이는 아직 명시적으로 검증되지 않았지만, 코드 구조상 논리적으로 따라오는 우려다.
...
- 다음: **D2-7. 이 프로젝트에서의 실측 비교와 튜닝 계획** — D2-1~D2-6에서 다룬 모든 개념이 실제로 어떤 수치로 나타났는지, 그리고 다음에 무엇을 검증할지 정리하는 이 시리즈의 마지막 문서다.
