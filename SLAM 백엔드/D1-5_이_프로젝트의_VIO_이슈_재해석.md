...
| 예상 선행 지식 | `SLAM 백엔드 - D1-1`(시스템 구조), `D1-2`(초기화 알고리즘), `D1-3`(Atlas 재추적·병합), `D1-4`(Place Recognition) 전체 |
...
| 관련 문서 | 이전: `SLAM 백엔드 - D1-4. Loop Closing과 Place Recognition` / 다음: `RTAB-Map & Nav2 심화 - C7. Relocalization 감시 로직 구현` |
...
5. 현재 미시도 변수 중 우선순위가 가장 높은 것은 **D435i IMU의 공장 출고 미보정(`rs-imu-calibration` 미실행)**이다 — D1-2에서 배운 2단계(Inertial-only MAP Estimation)의 입력 품질과 직결되기 때문이다.
...
| `rs-imu-calibration` | D435i IMU 공장 보정 — 현재 우선순위가 가장 높은 미시도 변수 | D1-2(2단계 입력 품질) |
...
* 다음 문서(`RTAB-Map & Nav2 심화 - C7`)는 SLAM 백엔드가 RTAB-Map이든 ORB-SLAM3든 공통으로 필요한 감시 로직을 다룬다 — 이 문서의 결론(ORB-SLAM3 VIO 미채택)에 따라, 그 구현은 현재 RTAB-Map 기준으로 유지된다.
...
- 미시도 변수: `rs-imu-calibration`(D435i IMU 공장 보정) — 8.3절 관점에서 보면, 2단계(Inertial-only MAP Estimation)의 입력 품질을 개선할 수 있는 유일하게 아직 시도되지 않은 변수이므로 우선순위가 높다.
- 다음 단계 후보(기존과 동일): 더 나은 관측 가능성(observability)을 가진 bag으로 재검증, 또는 IR 스테레오+IMU 기반 다른 R3 백엔드의 Jetson 적합성 확인.

**이 문서가 새로 제시하는 진단 우선순위 (D1-1~D1-4 근거 기반)**

1. **`rs-imu-calibration` 실행** — 8.3절에서 짚은 대로, 2단계 입력 품질 개선의 가장 직접적인 미검증 변수.
...
3. **성공 판정 기준 자체 조사** — "게이팅을 통과했는데도 재시도된다"는 것은, ORB-SLAM3 내부에 게이팅과 별개의 수렴 판정 로직(예: 스케일/중력 오차 임계값)이 있고 이것이 이 프로젝트 환경에서 유독 엄격하게 작동할 가능성을 시사한다 — 소스 레벨에서 이 판정 로직을 직접 확인하는 것이 다음 논리적 단계다.
...
4. D435i IMU가 공장 미보정 상태라는 점이, 2단계(Inertial-only MAP Estimation) 입력 품질 문제의 유력한 후보로 남아 있다.
5. 현재 결론은 ORB-SLAM3 RGB-D+IMU를 배포 후보로 채택하지 않는 것이며, 다음 우선순위는 `rs-imu-calibration` 실행이다.
...
5. 이 문서의 결론(ORB-SLAM3 RGB-D+IMU 미채택)은 왜 "확정된 결론"이 아니라 "현재까지의 진단 상태 기록"이라고 표현되는가?
...
4. `rs-imu-calibration`(D435i IMU 공장 보정)이다. 근거는 2단계(Inertial-only MAP Estimation)가 IMU 센서 노이즈 특성을 신뢰할 수 있어야 정확히 동작하는데, 이 센서가 아직 공장 보정을 거치지 않은 유일한 미검증 변수이기 때문이다.
5. 이 문서는 지금까지의 로그와 실험으로 원인을 좁힌 상태를 기록한 것일 뿐, `rs-imu-calibration` 등 아직 시도하지 않은 검증이 남아 있어 그 결과에 따라 결론이 달라질 수 있기 때문이다.
...
1. **바로 다음**: `RTAB-Map & Nav2 심화 - C7. Relocalization 감시 로직 구현` — 이 문서의 결론(ORB-SLAM3 VIO 미채택)에 따라, 현재 RTAB-Map 기준으로 유지되는 감시 로직 구현을 다룬다.
2. **함께 보면 좋은 주제**: `SLAM 백엔드 - D1-2. Visual-Inertial 초기화 알고리즘` — 이 문서의 8장에서 반복 인용한 3단계 알고리즘을 다시 복습하면 진단 근거가 더 명확해진다.
...
| 프로젝트 진단 노트 | `[[ros2-nav-yahboom]]` | 전체 진단 이력 원본, `IMU.fastInit` 구현 및 게이팅 격리 테스트 상세 로그 |
