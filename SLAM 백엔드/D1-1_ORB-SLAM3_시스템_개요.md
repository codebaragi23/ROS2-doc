...
| 예상 선행 지식 | `RTAB-Map & Nav2 심화 - A1. RTAB-Map 매핑 원리`(그래프 기반 SLAM, Loop Closure 개념), `ROS2 기초 - 07. TF2 기초`(좌표계 개념) |
...
| 관련 문서 | 이전: `RTAB-Map & Nav2 심화 - A1. RTAB-Map 매핑 원리` / 다음: `SLAM 백엔드 - D1-2. Visual-Inertial 초기화 알고리즘` |
...
4. 세 스레드는 하나의 통합된 place recognition 데이터베이스(DBoW2)를 공유하며, 이 데이터베이스가 재위치인정(relocalization)·루프 클로징·지도 병합에 공통으로 쓰인다.
...
- 통합된 place recognition 데이터베이스가 없다면, 재위치인정(relocalization)·루프 클로징·지도 병합을 각각 다른 방식으로 구현해야 해서 코드와 검증 대상이 불필요하게 늘어난다.
...
- ORB-SLAM3 자체는 이 문서에서 설명하는 세 스레드가 내부적으로 돌아가는 **하나의 ROS2 노드**로 실행된다. 앞선 `ROS2 기초` 시리즈에서 배운 "노드"라는 개념 그대로다.
...
| Local Mapping Thread | Active Map에 키프레임/포인트 추가, 지역 BA로 지도 정제, VI 모드에서 IMU 초기화 수행 | D1-2에서 다룰 IMU 초기화가 바로 여기서 일어남 |
...
3. **지도 정제**: Local Mapping Thread가 키프레임을 받아 포인트를 추가하고, 국소 영역에서 Bundle Adjustment로 지도를 다듬는다.
...
* 이 문서에서 배운 "그래프 최적화로 드리프트를 편다"는 개념은 `RTAB-Map & Nav2 심화 - A1`에서 배운 그래프 SLAM의 Loop Closure 개념과 원리상 같은 계열이다 — 다만 ORB-SLAM3는 이를 세 스레드로 명확히 분리하고 Atlas라는 다중 지도 구조를 얹었다는 점이 다르다.
* 이 문서의 "Tracking이 유실되면 relocalization을 시도한다"는 개념은 `RTAB-Map & Nav2 심화 - B8. Relocalization이 일어나는 위치`, `C1~C7 Relocalization 심화` 시리즈에서 다룬 개념과 대응된다. 다만 RTAB-Map은 단일 지도 안에서의 재위치인정을 다루는 반면, ORB-SLAM3는 "다른 지도"라는 경우의 수까지 포함한다는 점이 확장된 지점이다.
...
1. **바로 다음**: `SLAM 백엔드 - D1-2. Visual-Inertial 초기화 알고리즘` — 이 문서에서 짚은 "Local Mapping 스레드의 IMU 초기화"가 정확히 어떤 3단계 알고리즘으로 이루어지는지 다룬다.
2. **함께 보면 좋은 주제**: `RTAB-Map & Nav2 심화 - A1. RTAB-Map 매핑 원리` — 같은 그래프 SLAM 계열이지만 메모리 관리와 지도 구조가 다른 대안 백엔드를 복습하며 비교하면 좋다.
3. **나중에 학습할 심화 주제**: `SLAM 백엔드 - D1-4. Loop Closing과 Place Recognition` — 이 문서에서 "DBoW2 기반으로 매칭을 찾는다"고 간단히 언급한 부분을 알고리즘 수준까지 파고든다.
...
| 구분 | 자료 | 핵심 내용 |
|---|---|---|
| 논문 | Campos et al., "ORB-SLAM3: An Accurate Open-Source Library for Visual, Visual-Inertial and Multi-Map SLAM," IEEE T-RO 37(6), 2021 (arXiv:2007.11898) | 3장(System Overview) — 세 스레드 구조와 Atlas의 정의, 설계 목표 |
| 공식 저장소 | GitHub UZ-SLAMLab/ORB_SLAM3 | 인용 문헌 목록, 설정 파일(YAML) 항목(`System.LoadAtlasFromFile` 등) 구조 확인 |
