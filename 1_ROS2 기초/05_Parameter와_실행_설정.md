...
- LiDAR 00 드라이버는 `frame_id`, `scan_frequency`, `range_min`/`range_max` 같은 파라미터를 가진다. 실제 주행 환경에 따라 최소/최대 감지 거리를 조정해야 할 때 코드를 건드리지 않고 파라미터만 바꾼다.
...
* 앞으로 다룰 LiDAR 00, RealSense D435i 드라이버의 설정(해상도, 프레임 레이트, 프레임 ID 등)은 모두 이 문서에서 배운 파라미터 + YAML 구조를 그대로 사용한다.
...
5. 실제 LiDAR 00이나 RealSense D435i 드라이버에서 파라미터가 어떻게 활용될 것으로 예상되는가?
