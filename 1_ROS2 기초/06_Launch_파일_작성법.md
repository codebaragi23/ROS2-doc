...
- 즉 이 그림은 지금까지 배운 **노드 + 파라미터**를 **Launch 파일이라는 하나의 진입점**으로 묶는 구조를 보여준다. 실제 X3 로봇에서는 이 노드들이 LiDAR·카메라 드라이버, 모터 제어 노드가 된다.
...
3. **나중에 학습할 심화 주제**: [[05_Launch_시스템_디버깅|문제 해결 - Launch 시스템 디버깅]] — 노드가 여러 개로 늘어난 실제 로봇 환경에서 특정 노드만 실행이 안 되는 문제를 진단하는 방법을 다룬다.
