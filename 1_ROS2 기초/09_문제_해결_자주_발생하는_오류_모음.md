...
> 이 문서를 읽으면 0~08 전체 학습 과정에서 등장했던 오류들을 증상별로 빠르게 찾아 원인을 진단하고 해결할 수 있게 된다. 새로운 개념을 배우는 문서가 아니라, 지금까지 배운 것을 "문제가 생겼을 때 어떻게 찾아가는지" 관점에서 재구성한 참고용 문서다.
...
|---|---|
| 학습 단계 | Level 4 — 실전 진단 |
| 예상 선행 지식 | 0~08 전체 (개념보다는 각 문서의 10장 "자주 발생하는 문제"를 통합 정리) |
...
이 문서는 새로운 ROS2 개념이 아니라, **0~08에서 다룬 오류들을 증상 기준으로 재분류한 색인(index)**이다.
...
- 각 분기(Q1~Q4)는 지금까지 배운 문서(1, 3, 4, 5, 07)의 핵심 문제 유형과 정확히 대응된다. 즉 이 문서는 새로운 진단 방법을 제시하는 것이 아니라, 이미 배운 각 문서의 10장 내용을 하나의 흐름으로 재구성한 것이다.
- 마지막 단계(F5)에서도 원인이 안 잡히면, 08에서 배운 GUI 도구(`rqt_graph`, RViz2)로 시스템 전체를 시각적으로 훑어보는 것이 다음 단계다.
...
| 오류 범주 | 대표 증상 | 관련 문서 |
|---|---|---|
| 빌드/환경 오류 | `Package not found`, `colcon build` 실패 | 01 |
| 노드 실행 오류 | 노드가 `ros2 node list`에 안 뜸, 이름 충돌 | 02 |
| Topic 통신 오류 | Subscriber에 데이터 안 들어옴 | 03 |
| Service/Action 오류 | 응답 없이 멈춤, Feedback만 계속됨 | 04 |
| 파라미터 오류 | `Parameter not declared`, YAML 미적용 | 05 |
| Launch 오류 | 파일/패키지 못 찾음, 일부 노드만 죽음 | 06 |
| TF 오류 | `Could not find a connection`, RViz2 TF Error | 07 |
| 시각화 오류 | RViz2 빈 화면, Display 경고 아이콘 | 08 |
...
* 01, 03, 07의 실습 환경 (`my_first_pkg`, `simple_publisher`/`simple_subscriber`, TF 실습)
...
* 진단: `ros2 run tf2_tools view_frames`로 해당 좌표계가 아예 존재하지 않는 것을 확인한 뒤, Static Transform Broadcaster를 실행해서 해결한다. (07 실습 참고)
...
* `echo $AMENT_PREFIX_PATH`: 01에서 다룬 워크스페이스 등록 여부를 확인하는 핵심 환경 변수다. 이 안에 내 워크스페이스의 `install` 경로가 없다면 `source install/setup.bash`가 누락된 것이다.
...
이 장은 0~08에서 다룬 문제들을 증상 키워드 기준으로 재정리한 색인이다. 각 항목의 상세한 원인/해결은 해당 문서의 10장을 참고한다.
...
* 이 문서의 모든 항목은 새로운 원인이 아니라, 0~08 각 문서의 10장에서 이미 다룬 문제들을 증상 기준으로 재배열한 것이다. 즉 이 문서만 따로 외우기보다, 각 항목이 가리키는 원래 문서를 함께 참고하는 것이 정확한 이해에 도움이 된다.
* "빌드/source 누락"이 거의 모든 범주(1, 2, 3, 06)에서 공통적으로 첫 번째 의심 원인으로 등장하는 이유는, 01에서 배운 워크스페이스 구조가 ROS2 전체 시스템의 가장 기초적인 전제 조건이기 때문이다.
* "이름/타입 불일치"가 Topic(03), Service/Action(04), 파라미터(05), TF(07)에서 반복적으로 등장하는 이유는, ROS2 통신이 근본적으로 "이름 + 타입 기반 자동 연결"이라는 동일한 원칙(02에서 처음 언급) 위에서 동작하기 때문이다.
...
1. **바로 다음**: [[01_DDS와_QoS_이해하기|응용 01 - DDS와 QoS 이해하기]] — 이 문서에서 원인으로만 언급했던 **QoS 불일치의 실제 진단법**을 다룬다. 이후 응용 02·03에서 실제 센서를 연동한다.
