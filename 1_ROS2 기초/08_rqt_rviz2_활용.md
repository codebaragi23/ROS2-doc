...
- LiDAR 00과 RealSense D435i를 동시에 연결했을 때, RViz2 화면에 LiDAR 점군과 카메라 포인트클라우드가 서로 겹치지 않고 어긋나 있다면 TF 설정(7편에서 배운 `laser_link`, `camera_link` 위치값)이 잘못되었다는 것을 즉시 알 수 있다.
...
* 앞으로 다룰 [[02_센서_연동_RealSense_D435i|센서 연동 - RealSense D435i]], [[03_센서_연동_LiDAR_C1|센서 연동 - LiDAR 00]] 문서의 "확인" 단계는 대부분 이 문서에서 배운 RViz2 Display(Image, PointCloud2, LaserScan)를 활용한다.
...
2. **함께 보면 좋은 주제**: [[03_센서_연동_LiDAR_C1|센서 연동 - LiDAR 00]] — `LaserScan` Display를 활용해 실제 LiDAR 데이터를 시각적으로 검증하게 된다.
