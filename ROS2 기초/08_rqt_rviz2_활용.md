...
- LiDAR C1과 RealSense D435i를 동시에 연결했을 때, RViz2 화면에 LiDAR 점군과 카메라 포인트클라우드가 서로 겹치지 않고 어긋나 있다면 TF 설정(7편에서 배운 `laser_frame`, `camera_link` 위치값)이 잘못되었다는 것을 즉시 알 수 있다.
