...
01-02에서 다룬 두 방식을 다시 정리하면:

- **ORB-SLAM3(01)**: 키프레임과 map point로 이루어진 명시적 그래프를 Bundle Adjustment로 최적화. 지도가 계속 누적됨.
- **OpenVINS(02)**: 슬라이딩 윈도우 안의 pose를 EKF로 순차적으로 업데이트. 특징점은 상태에 거의 넣지 않음(null-space projection).
...
| 지도 누적 | Atlas로 계속 누적(01-3) | 없음(고정 윈도우, 02-4) | 없음(고정 윈도우) + 별도 pose graph로 장기 기억 |
...
- VINS-Fusion은 "실패 감지 및 복구" 기능을 공식적으로 표방하므로, 01(명시적 리셋)과 02(리셋 없음) 사이의 중간적 동작을 보일 가능성이 있다 — 실제 통합 전까지는 추정에 불과하다.
- Ceres Solver 기반 최적화는 매 프레임마다 반복 연산이 필요해, 02-4에서 다룬 OpenVINS의 "1-step 필터 업데이트"보다 구조적으로 CPU 비용이 높을 가능성이 있다(04에서 문헌 기반 비교).
