...
| 관련 문서 | 이전: 없음 (SLAM 공통 기초 시작점) / 다음: [[A0-1_SLAM_백엔드_종류_한눈에_보기\|A0-1. SLAM 백엔드 종류 한눈에 보기]] |
...
4. SLAM 시스템은 보통 **프론트엔드**(센서 데이터에서 짧은 구간의 움직임을 뽑아내는 부분)와 **백엔드**(그 움직임들을 모아 전체 지도의 일관성을 맞추는 부분)로 나뉜다. 뒤에 나올 D1~D4 시리즈가 "SLAM 백엔드 심화"라는 이름을 쓰는 이유가 바로 이 구분에서 나온다.
...
이 문서는 개념 이해가 목적이므로, 설치 없이 **SLAM이 실제로 만들어내는 결과물의 형태**를 눈으로 확인하는 실습으로 진행한다. 직접 로봇을 움직여 지도를 만드는 실습은 지상로봇 적용의 A1부터 다룬다.
...
이 문서는 아직 실행 코드나 설정 파일을 다루지 않는다. 특정 SLAM 라이브러리의 실제 설정과 코드 수준의 내용은 지상로봇 적용의 A1(RTAB-Map 매핑 원리)과 이후 SLAM 백엔드 심화 시리즈(D1. ORB-SLAM3 등)에서 본격적으로 다룬다.
...
SLAM이 만드는 `map` 좌표계는 "이 로봇이 처음 켜진 곳"을 기준으로 하는 상대적인 지도이며, 정확도는 Odometry 품질과 Loop Closure 성공 여부에 좌우된다는 점을 이해한다. 이는 지상로봇 적용의 A1에서 실제 파라미터와 함께 구체화된다.
...
* 지상로봇 적용의 A1은 이 문서에서 다룬 일반 SLAM 개념이 **RTAB-Map**이라는 구체적인 구현체에서 어떻게 현실화되는지(WM/STM/LTM 메모리 관리 등) 다룬다.
...
1. **바로 다음**: [[A0-1_SLAM_백엔드_종류_한눈에_보기|A0-1. SLAM 백엔드 종류 한눈에 보기]] — 지상로봇/드론 적용으로 갈라지기 전에, SLAM 백엔드를 구분하는 축을 가볍게 정리한다.
2. **함께 보면 좋은 주제**: [[07_TF2_기초|ROS2 기초 - 07. TF2 기초]] — `map`/`odom`/`base_link` 좌표계 트리를 복습하면 이 문서의 4장 그림이 더 명확해진다.
3. **나중에 학습할 심화 주제**: [[D1-1_ORB-SLAM3_시스템_개요|SLAM 백엔드 심화 - ORB-SLAM3 시스템 개요]] — 이 문서에서 예고한 "필터 기반 vs 최적화 기반" 구분이 실제 시스템 비교로 이어진다.
