...
> 지상로봇 적용 — Relocalization 심화 (Part C) 시리즈 · 보충
> 이전 문서: C2. 트리거 시점 5분류
...
C2에서 다룬 5가지 트리거 시점을, 이 프로젝트가 어떤 방식으로 감지하고 처리할 것인지 정리한다.

> [!warning] 구현 상태에 대한 주의
> **아래 ①~⑤는 대부분 아직 구현되지 않은 설계안이다.** Nav2도 RTAB-Map도 이런 트리거 감시를 기본 제공하지 않으므로, 실제로 동작하게 하려면 프로젝트가 직접 감시 노드를 만들어야 한다. 그 최소 골격을 실제 코드로 구체화한 것이 C7이며, **C7의 예제도 5개 트리거 중 ②만 다룬다.** 이 문서를 읽을 때 "지금 돌아가고 있는 시스템의 설명"이 아니라 "무엇을 만들어야 하는지의 명세"로 이해해야 한다.
...
- **분류**: 🔧 **프로젝트 커스텀 (Nav2 표준 기능을 재활용)**. B6에서 다뤘듯 Nav2 공식 `ReinitializeGlobalLocalization`은 AMCL 전용이라 이 프로젝트에는 적용되지 않는다. 여기서 쓰는 "Spin"은 B6에서 다룬 **Nav2 표준 Recovery 액션**을 relocalization 목적으로 빌려 쓰는 것이다 — Nav2 자체가 이 상황을 relocalization으로 인식하고 발동하는 것이 아니라, 프로젝트가 직접 감시 로직을 만들어 Spin 액션을 호출하는 구조여야 한다는 뜻이다.
...
- 포즈 공분산 — `map→odom` TF의 신뢰도(B2 2-1장 참고)

> **실무상의 제약**: 공분산은 RTAB-Map이 곧바로 쓰기 좋은 형태로 내주지 않는 경우가 많아, 커스텀 노드로 따로 추출해야 한다. **그래서 이 프로젝트는 실질적으로 `/rtabmap/info`의 inlier 수를 공분산의 대리 지표로 사용한다** — C7의 감시 노드 예제도 inlier만 본다. 위 ①·⑤에서 "공분산 검증"이라고 적은 판단 기준도 현실적으로는 inlier 기반으로 대체해 구현하게 된다.
