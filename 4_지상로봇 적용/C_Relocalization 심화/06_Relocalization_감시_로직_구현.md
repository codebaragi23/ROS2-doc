# 06. Relocalization 감시 로직 구현

> 지상로봇 적용 — Relocalization 심화 시리즈 (06, 확장 추가분)
> 이전 문서: [[05_Nav2_표준_relocalization과의_차이|05. Nav2 표준 relocalization과의 차이]]
> 이 문서는 01-1(이 프로젝트의 트리거 구현)에서 이론으로만 정리했던 감시 로직을 실제 ROS2 노드 설계 수준으로 구체화한 것이다. 코드는 개념 검증용 예시이며, 실제 배포 전 프로젝트 환경에서 검증이 필요하다.

## 문서 정보

| 항목 | 내용 |
|---|---|
| 학습 단계 | Level 3 — 심화 (코드, 시리즈 마무리) |
| 예상 선행 지식 | Relocalization 심화 00~05, [[06_Composable_Node와_Executor_구조\|응용 06]](Executor·데드락) |
| 학습 목표 | 감시 노드가 구독해야 할 정보를 안다 / 콜백 안 블로킹 대기가 왜 데드락인지 설명할 수 있다 / 중복 트리거를 막는 상태 플래그의 필요성을 이해한다 / 5가지 트리거 중 무엇이 추가 구현 대상인지 안다 |
| 기준 환경 | Yahboom ROSMASTER X3, ROS2 Humble (rclpy) |

## 1. 개요

01·02에서 정리한 5가지 relocalization 트리거는 Nav2나 RTAB-Map이 자동으로 감지해주지 않는다. 05에서 짚었듯, 이 감시 자체를 프로젝트가 직접 구현해야 한다. 이 문서는 그 구현의 최소 골격을 제시하며, C 시리즈(00~05) 전체의 이론을 실제 코드로 마무리하는 문서다.

## 2. 핵심 개념: 감시 노드가 구독해야 할 정보

| 정보 | Topic/서비스 | 어떤 트리거(01)와 관련되나 |
|---|---|---|
...
class RelocalizationMonitor(Node):
  def __init__(self):
    ...
    # Threshold constants — must be tuned per environment (see 02)
    self.MIN_INLIERS = 15
  ...
  def info_callback(self, msg: Info):
    ...
    # 01 trigger ②: emergency relocalization when tracking looks lost
...
> **이 골격이 다루는 범위**: 위 코드는 01의 5가지 트리거 중 **② 위치 추정 유실**만 구현한다. ①(초기 기동)은 AprilTag 검출 노드 입력이, ③(누적 드리프트)은 주행 거리 누적 추적이, ④(랜드마크 진입)는 마커 pose 구독이, ⑤(정밀 작업 직전)는 Nav2 목표 진입 상태 감시가 각각 추가로 필요하다.

## 4. 관련 파라미터/설계 고려사항

| 항목 | 고려사항 |
|---|---|
| 임계값(`MIN_INLIERS`, `LOST_THRESHOLD`) | 너무 민감하면 정상 흔들림에도 오발동(03의 가속도 게이팅과 함께 검토) |
| Spin 발동 중복 방지 | 이미 Spin recovery가 진행 중일 때 중복 트리거되지 않도록 상태 플래그 필요 |
| 01 ⑤(정밀 작업 직전) 로직 | 이 감시 노드와 별개로, 도킹 시작 직전 별도의 "정지-검증" 상태 머신이 필요(원문 5번 항목) |
...
- Spin recovery가 예상보다 자주/드물게 발동한다면 `MIN_INLIERS`/`LOST_THRESHOLD`를 04(실전 진단 체크리스트)의 기록 항목과 함께 실험적으로 조정한다.
...
- [[00-1_Nav2_Action_기반_경로_이동_구조|지상로봇 적용 - Nav2 내비게이션 - 00-1. Nav2 Action 기반 경로 이동 구조]] — `Spin` Action 클라이언트 사용법
- [[01_트리거_시점_5분류|지상로봇 적용 - Relocalization 심화 - 01. 트리거 시점 5분류]], [[01-1_이_프로젝트의_트리거_구현|지상로봇 적용 - Relocalization 심화 - 01-1. 이 프로젝트의 트리거 구현]], [[02_RTAB-Map_핵심_파라미터_대응표|지상로봇 적용 - Relocalization 심화 - 02. RTAB-Map 핵심 파라미터 대응표]], [[07_Relocalization이_일어나는_위치|지상로봇 적용 - Nav2 내비게이션 - 07. Relocalization은 Nav2 파이프라인의 어디에 위치하는가]] — 이 구현의 이론적 근거
