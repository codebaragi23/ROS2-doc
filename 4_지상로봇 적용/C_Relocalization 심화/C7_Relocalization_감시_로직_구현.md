...
> 지상로봇 적용 — Relocalization 심화 (Part C) 시리즈 (C7, 확장 추가분)
...
- 이것으로 Relocalization 심화(Part C, C1~C7)가 완결된다. 지도제작(Part A) → Nav2 내비게이션(Part B) → Relocalization 심화(Part C)로 이어지는 **지상로봇 적용** 흐름이 여기서 마무리되며, 다음은 지금까지의 스택이 드론에서 어떻게 달라지는지 다루는 **[[E0_지상로봇과_드론은_무엇이_다른가|드론 적용 (Part E)]]**로 이어진다. SLAM 백엔드 자체를 더 깊게 비교하는 [[D0-1_D_시리즈를_위한_최소_수학|SLAM 백엔드 심화 (Part D)]]는 지상로봇·드론 적용을 모두 읽은 뒤 보는 것을 권한다.
- 향후 이 감시 노드가 실제로 구현·검증되면, 결과를 C5(실전 진단 체크리스트)에 반영하는 별도 갱신이 필요하다.
- 별도 심화가 필요한 대안 SLAM 백엔드 논의(ORB-SLAM3, OpenVINS, VINS-Fusion)는 독립된 "SLAM 백엔드 심화" 시리즈(D1~D4)에서 다룬다.
