# 02. RTAB-Map 핵심 파라미터 대응표

> 지상로봇 적용 — Relocalization 심화 시리즈
> 이전 문서: [[01-1_이_프로젝트의_트리거_구현|지상로봇 적용 - Relocalization 심화 - 01-1. 이 프로젝트의 트리거 구현]]
...
|---|---|
| 학습 단계 | Level 3 — 심화 (참조) |
| 예상 선행 지식 | [[01-1_이_프로젝트의_트리거_구현\|Relocalization 심화 01-1]] |
| 학습 목표 | 로컬라이제이션 모드에서 각 파라미터가 하는 역할을 안다 / 매핑 모드와 같은 이름이라도 역할이 다를 수 있음을 이해한다 / 증상별로 어떤 파라미터를 볼지 찾을 수 있다 |
| 기준 환경 | Yahboom ROSMASTER X3, RTAB-Map |

## 1. 개요

[[01-1_이_프로젝트의_트리거_구현|지상로봇 적용 - Relocalization 심화 - 01-1. 이 프로젝트의 트리거 구현]]에서 트리거별로 흩어져 언급한 RTAB-Map 파라미터들을 relocalization 관점에서 한곳에 모아 정리한다. 01(매핑 파라미터)와 겹치는 이름이 있지만, 여기서는 **로컬라이제이션 모드에서의 역할**에 집중한다.
...
| 파라미터 | 기본값/예시 | 역할 | 01-1의 어느 트리거와 관련되나 |
|---|---|---|---|
...
| `RGBD/OptimizeFromGraphEnd` | `false`(기본) | true로 설정 시 로봇 pose 대신 지도(map) 쪽이 점프하도록 전환 — 03에서 자세히 | 안정화(03) |
...
- `RGBD/SavedLocalizationIgnored: true`는 랜드마크 재진입 시 강한 보정을 만들지만, 저장된 localization을 매번 무시하므로 **평소 안정적인 추적 상태에서도 불필요하게 자주 relocalization이 발동**할 수 있다. 이 값은 상시 true로 두기보다, 01-1 ④번 트리거 조건(랜드마크 진입)이 성립할 때만 일시적으로 적용하는 방식이 더 안전하다.
...
- "relocalization 자체는 맞는데 로봇이 덜컥거린다" → 이건 파라미터 문제가 아니라 03(Pose Jump 제어)의 영역이다.
