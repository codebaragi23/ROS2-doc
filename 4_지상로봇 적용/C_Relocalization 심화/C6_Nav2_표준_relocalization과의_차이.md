...
> 지상로봇 적용 — Relocalization 심화 (Part C) 시리즈
> 이전 문서: C5. 실전 진단 체크리스트

## 1. 개요

B6·C1에서 반복해온 "Nav2 표준 relocalization(AMCL 전용) vs 이 프로젝트의 RTAB-Map 커스텀 전략"의 차이를 한곳에 정리하고, 향후 구조 개선 방향을 짚는다.
...
- **감시 로직의 노드화**: C2의 5분류를 실제로 자동 판단하려면, `/rtabmap/info`(inlier 수)와 공분산을 구독해 트리거 조건을 판단하는 전용 ROS2 노드가 필요하다. 현재는 이 로직이 문서(전략)로만 존재하며, 이를 실제 ROS2 노드 설계로 구체화하는 것이 다음 문서 C7의 주제다.

## 6. 다음 문서와의 연결

- 다음: **C7. Relocalization 감시 로직 구현** — C5에서 언급한 "감시 로직 노드화"를 실제 ROS2 노드 설계로 구체화하며 Part C를 마무리한다.
- ORB-SLAM3/OpenVINS/VINS-Fusion처럼 [[ros2-nav-yahboom]]에 기록된 대안 SLAM 백엔드 검토는 별도의 "SLAM 백엔드 심화" 시리즈(D1~D4)에서 다룬다.
