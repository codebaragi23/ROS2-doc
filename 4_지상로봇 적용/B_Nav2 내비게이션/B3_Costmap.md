...
> **layer와 filter는 다르다**: 위 목록은 전부 **레이어(layer)**로, 센서·지도 정보를 costmap에 쌓는 것들이다. 이와 별개로 **Costmap Filter**라는 계열이 있는데(속도 제한 구역, 진입 금지 구역 등), 이는 별도의 마스크 지도를 추가로 요구한다 — C4에서 `speed_limit` 필터를 다룰 때 다시 나온다.
