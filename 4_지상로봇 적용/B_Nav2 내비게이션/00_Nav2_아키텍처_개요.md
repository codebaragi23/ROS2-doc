...
> 이전 문서: [[02_Costmap|02. 매핑 품질 진단]]

## 문서 정보

| 항목 | 내용 |
|---|---|
| 학습 단계 | Level 2 — 적용 (Nav2 시작점, 실습) |
| 예상 선행 지식 | [[02_매핑_품질_진단\|지도제작 02]](지도 준비 완료), [[06_Launch_파일_작성법\|ROS2 기초 06]] |
| 학습 목표 | Nav2의 주요 서버 노드와 각 역할을 말할 수 있다 / lifecycle 노드 개념을 이해한다 / 목표를 한 번 보내 로봇을 실제로 이동시킬 수 있다 / 안 움직일 때 어느 서버부터 확인할지 안다 |
| 기준 환경 | Yahboom ROSMASTER X3, Nav2 (ROS2 Humble) |

## 1. 개요

지도제작에서 만든 지도를 가지고 실제로 로봇을 목적지까지 움직이는 것이 Nav2의 역할이다. 이 문서는 Nav2 전체 구조를 큰 그림으로 먼저 잡고, 01~06에서 각 구성요소를 하나씩 깊게 다룬다.
...
|---|---|---|
| `bt_navigator` | 전체 행동 흐름(경로 계획→추종→도착 판정→recovery) 관리 | 05 |
| `planner_server` | 전역 경로 생성 | 03 |
| `controller_server` | 로컬 경로 추종, 실제 속도 명령 생성 | 04 |
| `behavior_server` | recovery 동작(spin/backup/wait 등) 실행 | 05 |
| `waypoint_follower` | 여러 목표점을 순차 이동 | 06 |
| `costmap` (global/local) | 장애물 회피용 비용 지도 | 02 |
| `velocity_smoother` | 최종 속도 명령 완화 | 06 |

이 목록에 **AMCL이 없다는 점**이 중요하다. Nav2 표준 예제(nav2_bringup)는 대부분 AMCL을 로컬라이제이션으로 쓰지만, 이 프로젝트는 **RTAB-Map localization**이 `map→odom` TF를 대신 채워준다. Nav2 입장에서는 이 TF가 어디서 오든 상관없이 소비만 하므로 위 서버 목록에서 로컬라이제이션 자체는 별도로 존재하지 않는다 — 이 구조는 01에서 자세히 다룬다.
...
전체 구조를 결정하는 상위 설정들만 정리한다. 세부 파라미터는 각 담당 문서(02~06)에서 다룬다.
...
Nav2 내비게이션 전체가 "목표를 주면 로봇이 간다"는 흐름을 다루므로, **가장 단순한 형태로 한 번 성공시켜 보는 것**이 이후 02~06을 읽는 기준점이 된다.

### 준비 사항

* 지도제작 01 실습에서 만든 지도, 01 실습의 로컬라이제이션 모드 실행 상태
* Nav2 실행 중
...
* `--feedback`을 붙이면 남은 거리 등 진행 상황이 실시간으로 출력된다 — 00-1에서 다룰 Action 피드백이 실제로 흐르는 것을 볼 수 있다.

### 예상 결과

| 결과 | 의미 | 다음에 볼 문서 |
|---|---|---|
| 로봇이 목표까지 이동 | 파이프라인 전체가 정상 | 02부터 순서대로 |
| `Goal was rejected` | 서버가 `active`가 아니거나 목표가 지도 밖 | 4-1절 |
| 경로는 나오는데 안 움직임 | 컨트롤러 또는 costmap 문제 | 02, 04 |
| 경로 자체가 안 나옴(`plan=0`) | 목표가 장애물/미탐색 영역에 있음 | 02, 03 |
...
- 다음: **[[01_좌표계와_로컬라이제이션_기반|지상로봇 적용 - Nav2 내비게이션 - 01. 좌표계와 로컬라이제이션 기반]]** — 이 문서에서 "RTAB-Map localization이 별도 서버 없이 TF만 채워준다"고 언급한 부분을 자세히 다룬다.
- Relocalization 심화는 이 00의 흐름도 중 "RTAB-Map localization" 박스 안에서 일어나는 일을 깊게 파고드는 것이라고 생각하면 된다.
