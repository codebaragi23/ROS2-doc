# 04. Controller — 경로 추종 (ROS1의 Local Planner)
...
| 증상 | 확인 순서 |
|---|---|
| 좁은 통로에서 멈춤 | local/global costmap이 실제로 열려 있는지 → global planner 경로 생성 여부 → MPPI 후보가 장애물 비용으로 모두 막히는지 → `ObstaclesCritic.repulsion_weight`/`cost_scaling_factor`/`collision_margin_distance` → local/global `inflation_radius`(02) → `progress_checker.movement_time_allowance`(05) |
| 장애물에 너무 붙음 | robot radius(02)가 실제보다 작지 않은지 → footprint/센서 위치 정합 → local inflation radius → `repulsion_weight` |
| 움직임이 거침 | MPPI 탐색 노이즈(`*_std`), 가속 한계, velocity smoother(06) |
