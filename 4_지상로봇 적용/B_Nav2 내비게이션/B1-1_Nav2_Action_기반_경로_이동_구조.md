# B1-1. Nav2 Action 기반 경로 이동 구조

> 지상로봇 적용 — Nav2 내비게이션 (Part B) 시리즈 · 보충
...
- B1에서 다룬 "Nav2가 여러 서버로 나뉜 구조"는 이 문서 관점에서 보면 **각 서버가 하나 이상의 Action 서버를 제공하는 구조**라고 다시 설명할 수 있다.
...
- 이 문서는 B6 바로 다음 보충 자료이며, 이후 흐름(B7, B8, Part C)은 기존과 동일하게 이어진다.
