...
> 지상로봇 적용 — Nav2 내비게이션 (Part B) 시리즈 (마지막 문서)
> 이전 문서: B7. Waypoint Follower & Velocity Smoother

## 1. 개요

Part B에서 다룬 bt_navigator, planner, controller, costmap, recovery는 전부 **`map→odom→base_footprint` TF가 항상 정확하다는 전제 위에서** 동작한다. 이 문서는 Part B 전체를 마무리하며, "그 전제를 실제로 채워주는 것이 무엇인가"를 짚고 Part C(Relocalization 심화)로 넘어가는 다리 역할을 한다.

## 2. 핵심 개념: Nav2 파이프라인과 분리된 별도 계층

```mermaid
flowchart TB
    subgraph Nav2["Nav2 파이프라인 (Part B)"]
        BT[bt_navigator] --> PL[planner_server]
        PL --> CO[controller_server]
        CO --> VS[velocity_smoother]
    end
    subgraph Loc["RTAB-Map Localization (Part C)"]
...
지금까지 Part A, B에서 다룬 내용을 relocalization 관점으로 다시 연결하면:

- A1~A3(지도제작): RTAB-Map DB가 정확할수록, 이후 로컬라이제이션 모드에서 재매칭할 "기준"이 튼튼해진다. 지도 품질이 relocalization 안정성의 전제 조건이다.
- B2(좌표계): `Mem/IncrementalMemory=false`로 로컬라이제이션 모드에 들어가야 RTAB-Map이 "새 지도를 만드는 것"이 아니라 "기존 지도에서 내 위치를 찾는 것"으로 동작한다.
- B3(Costmap): relocalization 중 `map→odom`이 순간적으로 크게 바뀌면 costmap 장애물 위치가 왜곡되어 급제동이 걸릴 수 있다(Part C에서 자세히).
- B6(Recovery): Spin recovery가 Nav2 표준 기능이면서 동시에 이 프로젝트의 relocalization 전략의 실행 수단이 된다는 이중적 위치를 이해해야 한다.
...
로봇 동작이 이상할 때, 이것이 **Part B(Nav2 파이프라인) 문제인지 Part C(RTAB-Map localization) 문제인지**부터 나누는 것이 가장 중요한 첫 진단 단계다.

- 로봇이 경로를 못 찾거나, 장애물 앞에서 계속 멈춘다 → Part B 관련(costmap, planner, controller)
- 지도 위에서 로봇 아이콘이 갑자기 다른 곳으로 튄다, 방향이 순간적으로 90도씩 바뀐다 → Part C 관련(localization jump)
- 두 증상이 같이 나타난다 → 대개 후자(Part C)가 원인이고 전자는 그 결과다. `map→odom` TF의 연속성부터 확인한다.

## 6. 다음 문서와의 연결

- Part B가 여기서 끝나고, **Part C. Relocalization 심화**가 시작된다.
- 다음 문서: **C1. 개요와 학술적 정의** — relocalization을 "Kidnapped Robot Problem"이라는 학술적 틀로 다시 정의하고, 이 프로젝트의 5단계 트리거 분류가 그 틀의 어디에 해당하는지 정리한다.
